...
const CouponCard: React.FC<CouponCardProps> = ({ coupon }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    cardRef.current.style.transform =
      `perspective(1000px) rotateX(${-y * 20}deg) rotateY(${x * 20}deg) scale3d(1.04, 1.04, 1.04)`;
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col h-full justify-between items-center text-center">
          <div className="flex-shrink-0">
            <div className="text-6xl mb-4" style={{ transform: 'translateZ(50px)' }}>
              {coupon.illustration}
            </div>
            <h3
              className="text-balance font-cinzel text-xl font-bold text-white"
          ...
          <p
            className="text-pretty text-sm text-white/80 leading-relaxed"
          ...
          <button
            ...
            disabled={isRedeemed}
            aria-label={isRedeemed ? 'Redeemed' : `Redeem coupon: ${coupon.title}`}
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center animate-fade-in">
          <div className="bg-gradient-to-br from-gray-800 to-black border border-rose-500 p-8 rounded-xl text-center shadow-2xl shadow-rose-500/30 max-w-sm mx-4">
            ...
            <div className={`text-5xl mt-4 ${prefersReduced ? '' : 'animate-ping'}`}>💌</div>
          </div>
        </div>
      )}
